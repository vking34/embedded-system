...
def get_command(current_direction, next_direction):
    command = []
    move_left_combinations = [(directions.NORTH, directions.WEST), (directions.WEST, directions.SOUTH), \
        (directions.SOUTH, directions.EAST), (directions.EAST, directions.NORTH)]
    move_right_combinations = [(directions.NORTH, directions.EAST), (directions.EAST, directions.SOUTH),\
...
def get_path(starting_point, target_point): # path must follow the line ( no diagonal )
    astar_path = astar(maze, starting_point, target_point)
    # print(astar_path)
    i = 0
    point = astar_path[i]
...
def run(starting_point, starting_headto_point, target_point):
    command_list = []
    initial_direction = get_direction(starting_point, starting_headto_point)
    path = get_path(starting_point, target_point)
    ...
    #         if command_list[i+1] == ['move_left', 'move_forward']:
    #             command_list[i+1] = ['move_right', 'move_forward']
    #         elif command_list[i+1] == ['move_right', 'move_forward']:
    #             command_list[i+1] = ['move_left', 'move_forward']
...
def main():
    ...
    # if not target_point[0] in [0, 5] and not target_point[1] in [0, 3]:
    #     print('invalid destination point.')
    #     return
    #
    command_list, path = run(starting_point, starting_headto_point, target_point)
    print(command_list)
    print(path)
